...
// Function to get message from update, updated to include business_message
function getMessageFromUpdate(update) {
  if (update.callback_query) {
    return update.callback_query.message;
  }
  // Check for business_message first
...
// Function to handle business_connection updates
async function handleBusinessConnection(update) {
  ...
  const chatId = userId; // Assuming the chat ID is the user's ID

  if (businessConnection.is_connected) {
    // Store the business_connection_id in Redis
    await redis.set(`business_connection_id:${chatId}`, businessConnection.business_connection_id);
    console.log(`Stored business_connection_id for user ${userId}`);
  } else {
    // Remove the business_connection_id from Redis
    await redis.del(`business_connection_id:${chatId}`);
    console.log(`Removed business_connection_id for user ${userId}`);
  }
}

// Function to handle business messages
async function handleBusinessMessage(msg) {
  ...
  try {
    ...
    // Generate a response using your existing models
...
// Modify handleMessage function to prioritize business messages and handle business_connection updates
async function handleMessage(update) {
  ...
  // Prioritize business_message
  if (update.business_message) {
    const msg = update.business_message;
    await handleBusinessMessage(msg);
    return; // Do not process other messages until business messages are handled
  }
...
// Modify other functions to use sendMessageWithFallback and sendPhotoWithFallback
